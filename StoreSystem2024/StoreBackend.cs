namespace StoreSystem2024 {
    internal class StoreBackend {
        ...
        // The backend destructor wasn't called. So instead I will rely on the frontent destructor to call the *WriteFile* function on the backend.
        /*~StoreBackend() {
            MessageBox.Show("TRYING TO WRITE TO FILE!");
            WriteFile(itemList);
        }*/
        ...
        public void addItem(string name, int price, int count, Dictionary<string, string> attributes) {
            //Figuring out a valid ID feels like the job of the backend...
            int i = 1;
            int[] ids = getAllIDs();
            while (true) {
                if (!ids.Contains(i)) {
            ...
            itemList.Add(i, new StoreItem(name, price, count, attributes));
        }
        ...
        public void addCount(int ID, int count) {
            StoreItem temp = itemList[ID];
            temp.count += count;
            itemList.Remove(ID);
            itemList.Add(ID, temp);
        }
